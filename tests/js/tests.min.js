...
var testBase = function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee() {
    var id, slider, info, controlsClick, navClick, controlsKeydown, navKeydown, testGoto, assertion, slideItems, visibleNavIndexes, len, i, current, currentSlide, prev, absIndex, wrapper, slideCount, navContainer, navItems, wrapperLeft, controls, input, button, mul, checkGoto;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            id = 'base', slider = sliders[id], info = slider.getInfo();
          ...
          case 13:
            ...
            // nav click
            slideItems = info.slideItems, visibleNavIndexes = info.visibleNavIndexes, len = visibleNavIndexes.length;
            ...
            for (i = len; i--;) {
              ...
              if (assertion !== false) {
                assertion = info.navItems[visibleNavIndexes[i]].getAttribute('aria-selected') === 'true' && current % info.slideCount === visibleNavIndexes[i] && compare2Nums(currentSlide.getBoundingClientRect().left, 0) && currentSlide.getAttribute('aria-hidden') === 'false';
              }
            }
          ...
          case 19:
            prev = info.index, current = slider.getInfo().index, absIndex = getAbsIndex(prev, -3, info), currentSlide = info.slideItems[current], wrapper = info.container.parentNode;


            assertion = current % info.slideCount === absIndex && info.navItems[absIndex].getAttribute('aria-selected') === 'true' && compare2Nums(currentSlide.getBoundingClientRect().left, wrapper.getBoundingClientRect().left);
          ...
          case 24:
            current = slider.getInfo().index, absIndex = 0, currentSlide = info.slideItems[current], wrapper = info.container.parentNode;
          ...
          case 26:
            ...
            // nav keydown
            assertion = undefined;
            info = slider.getInfo(), slideCount = info.slideCount, navContainer = info.navContainer, navItems = info.navItems, slideItems = info.slideItems, visibleNavIndexes = info.visibleNavIndexes, wrapperLeft = info.container.parentNode.getBoundingClientRect().left;
            ...
            if (assertion) {
              assertion = current % slideCount === visibleNavIndexes[1] && info.navItems[visibleNavIndexes[1]].getAttribute('aria-selected') === 'true' && compare2Nums(currentSlide.getBoundingClientRect().left, wrapperLeft);
            }
            ...
            if (assertion) {
              assertion = current % slideCount === visibleNavIndexes[2] && info.navItems[visibleNavIndexes[2]].getAttribute('aria-selected') === 'true' && compare2Nums(currentSlide.getBoundingClientRect().left, wrapperLeft);
            }
            ...
            if (assertion) {
              assertion = current % slideCount === visibleNavIndexes[0] && info.navItems[visibleNavIndexes[0]].getAttribute('aria-selected') === 'true' && compare2Nums(currentSlide.getBoundingClientRect().left, wrapperLeft);
            }
          ...
          case 52:
            ...
            // go to
            slideCount = info.slideCount, controls = document.querySelector('#base_wrapper .goto-controls'), input = controls.querySelector('input'), button = controls.querySelector('.button'), mul = 100;
            _context.next = 55;
...
function checkControlsClick(test, id, count, vertical) {
  ...
  var promise = id === 'customize' ? repeat(function () {
    simulateClick(info.prevButton);
  }, count) : repeat(function () {
    info.prevButton.click();
  }, count);
  ...
  return promise.then(function () {
    ...
    if (assertion) {
      var clickNext = id === 'customize' ? repeat(function () {
        simulateClick(info.nextButton);
      }, count) : repeat(function () {
